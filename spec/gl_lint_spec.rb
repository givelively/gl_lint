...
RSpec.describe GlLint do
  ...
  describe 'call_cli' do
    ...
    it 'parses the options' do
      expect_any_instance_of(GlLint::Linter).to receive(:lint).with(linters: %w[rubocop prettier], target_files: '--changed', filenames: nil, lint_strategy: :list_only)
      described_class.call_cli(app_root:)
    end
    context 'with a filename' do
      let(:passed_args) { ["--no-fix", "spec/gl_lint_spec.rb"] }
      it 'parses the options' do
        expect_any_instance_of(GlLint::Linter).to receive(:lint).with(linters: %w[rubocop prettier], target_files: '--changed', filenames: ['spec/gl_lint_spec.rb'], lint_strategy: :no_fix)
        described_class.call_cli(app_root:)
      end
    end
